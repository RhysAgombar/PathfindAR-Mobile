...
Java_com_example_raven_pathfindar_MainActivity_detectMarkers(JNIEnv *env, jobject instance, jlong image_final, jlong image_editable) {
    ...
    //cv::resize(inMat, inMat, cv::Size(720, 1280), 0, 0, cv::INTER_CUBIC);

/*
    float xScale = 720.0f/480.0f;
    float yScale = 1280.0f/640.0f;

    for (int i = 0; i < corners.size(); i++){
        for (int j = 0; j < corners.at(i).size(); j++){
            corners.at(i).at(j).x *= yScale;
            corners.at(i).at(j).y *= xScale;
        }
    }
*/
    ...
    if (start == true){
        ...
        if (ids.size() > 0) {
            ...
            cv::circle(finalMat, gridCorners.at(0), 5, cv::Scalar(255, 0, 0), -1);
            cv::circle(finalMat, gridCorners.at(1), 5, cv::Scalar(255, 0, 0), -1);
            cv::circle(finalMat, gridCorners.at(2), 5, cv::Scalar(255, 0, 0), -1);
            cv::circle(finalMat, gridCorners.at(3), 5, cv::Scalar(255, 0, 0), -1);
        }

        cv::circle(finalMat, arUco[0].center, 5, cv::Scalar(255, 0, 255), -1);
        cv::circle(finalMat, arUco[1].center, 5, cv::Scalar(255, 0, 255), -1);
        cv::circle(finalMat, arUco[2].center, 5, cv::Scalar(255, 0, 255), -1);
        cv::circle(finalMat, arUco[3].center, 5, cv::Scalar(255, 0, 255), -1);

        drawGrid(finalMat, gridCorners, hSize, vSize);

        // refine findTokens
        findTokens(finalMat, finalMat);

    }

    //cv::aruco::drawDetectedMarkers(inMat,corners,ids);
    //cv::aruco::drawDetectedMarkers(finalMat,corners,ids);
